...
cascade = base / "backend" / "auth" / "haarcascade_frontalface_default.xml"
...
while True:
    ...
    for (x,y,w,h) in faces:
        ...
        samples_dir = base / "backend" / "auth" / "samples"
        filename = f"face.{face_id}.{count}.jpg"
        cv2.imwrite(str(samples_dir / filename), converted_image[y:y+h,x:x+w])
